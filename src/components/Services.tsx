      ...
      <div ref={revealRef} className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid md:grid-cols-3 gap-8 items-stretch">
          {services.map((service, i) => (
            <div
              key={service.title}
              className={`reveal reveal-delay-${i + 1} rounded-2xl p-8 transition-all duration-300 hover:-translate-y-2 hover:shadow-2xl ${
                service.featured
                  ? "bg-sapphire-900 text-white shadow-xl ring-2 ring-gold/30 scale-[1.02]"
                  : "bg-white shadow-lg hover:shadow-xl"
              }`}
            >
              {service.featured && (
                <div className="inline-block bg-gold text-white text-xs font-bold px-3 py-1 rounded-full mb-4 tracking-wide uppercase">
                  Most Popular
                </div>
              )}
              <div
